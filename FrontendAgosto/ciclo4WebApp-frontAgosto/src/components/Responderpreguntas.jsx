...
export default function Responderpreguntas(props) {
    ...
    const fetchData = async () => {
		...
		if (idsPreguntasRes === "") {
            preguntasR = await Axios.get('pregunta/obtenerpreguntas', {
                headers: {'autorizacion': token},
        })
            ...
            if (comp === "todasComp"){
                preguntasR = await Axios.get('pregunta/buscarpregsest', {
                    headers: {'autorizacion': token},
                    params: {
                        ids: arraypreguntas3
                      },
                ...
                preguntasR = await Axios.get('pregunta/buscarpregsestcat', {
                    headers: {'autorizacion': token},
                    params: {
                        ids: arraypreguntas3,
                        comp: comp
			...
			// const preguntasR = await Axios.get('pregunta/buscarpregsest', {
            //     headers: {'autorizacion': token},
            //     params: {
            //         ids: arraypreguntas3
            //       },
    ...
    const guardarRespuestas = async (e) => {
      ...
      arrayresultados.forEach((resul, keys) => {            
            const getData = async () => {
                try {
                    return await Axios.post('resultado/guardar', resul,
                        {
                            headers: {'autorizacion': token}
                        })
                } catch (error) {
                                                    ...
                                                    <td>
                                                     <label>
                                                        <input 
                                                            name = {pregunta._id} 
                                                            type = "radio"
                                                            value = {pregunta._id + "*A"} 
                                                            onChange = {(event) => setRespuestas(respuestas.concat(event.target.value))}
                                                        />
